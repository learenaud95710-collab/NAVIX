...
// localStorage garde la session active même si l'utilisateur ferme la page.
function login(){
...
// Connexion téléphone : dans cette V1, le SMS est simulé.
// Dans une vraie app, on utiliserait Firebase Phone Auth, Twilio ou un backend.
function sendSmsCode(){
  ...
  // Affichage volontaire pour le prototype : cela remplace le SMS réel.
...
function createMission(){
  ...
  const robotCommand=`STATUS
GOTO:${mission.targetX},${mission.targetY}
OPEN`;

  robotCommandBox.textContent=robotCommand;
  missionCreated=true;

  console.log("Mission NAVIX créée :",mission);
  console.log("Commande robot envoyée :",robotCommand);
...
function openRobot(){
  ...
  openMessage.textContent=userCode===recoveryCode
    ? "Compartiment ouvert : code vérifié avec succès."
    : "Code incorrect : ouverture refusée.";
}

function formatPrice(value){return value.toFixed(2).replace(".",",")+" €";}

// Raccourcis DOM utilisés dans le script
...
// Au chargement, on vérifie si l'utilisateur avait déjà une session active.
